...
Maxvlaues1<-Maxvlaues %>%unite("chemunit",Chem,AquiferUnit,sep = "_")  

for(i in unique(Maxvlaues1$chemunit)){
  Maxvlaues<-subset(Maxvlaues1, chemunit == i)  
...
Basemap<-tm_shape(chbound)+tm_polygons()+
  ...
  tm_legend(legend.outside=TRUE) +tm_layout(title = paste("Sample Value"))
Basemap
...
tmap_save(tm=Compare,filename = paste0(i,"_MassMap_Max.jpg"),width = 20,height=20,units = "cm")
}